#!/usr/bin/env perl
#
# chbs - correct horse battery staple, a random passphrase generator.
...
B<chbs> - correct horse battery staple
...
B<chbs> generates passphrases comprised of randomly chosen words. A passphrase
of four to six words is reasonably strong, and much easier to remember and
to type than a sequence of random characters.
...
my @wordlist=( );	# The word list chosen by the user.
my $passphrase='';	# The passphrase constructed.

# Parameters
my $num=4;	# Number of words in the passphrase.
...
# Fatal errors
die "Invalid parameter: max must be greater than min.\nDied"
	if $max < $min;
die "Invalid parameter: positive numbers only!\nDied"
...
sub arbitraryFile {
	...
	# Use a hash to check for unique items. From Perl Cookbook,
	# Second Edition, Recipe 4.7.
	foreach my $item (@temp) {
		push(@wordlist, $item) unless $seen{$item}++;
	}

	return @wordlist;
}

# Use the the system spelling dictionary word list. This is really just a
...
sub systemDictionary {
	...
	close($SYSTEMDICTIONARY);

	return @wordlist;
}

# The default word list, included as a "here document", is a list of 3991
# common words in American English, compiled from word lists and scripts
# from "Spell Checker Oriented Word Lists" (SCOWL), hosted at
# 	http://wordlist.sourceforge.net/
# using
# 	mk-list english american 10 | grep -E '^[[:alpha:]]+$'
sub defaultWordList {
	# Generate the array
	push(@wordlist,split('\n',<<DEFAULTWORDLIST));
American
